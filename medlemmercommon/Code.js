var _MEDLEMMER_SHEET_ID = PropertiesService.getScriptProperties().getProperty(
  "MedlemmerSheetId"
);
...
function getMedlemmerSheet() {
  return SpreadsheetApp.openById(_MEDLEMMER_SHEET_ID).getSheetByName(
    _MEDLEMMER_SHEET_NAME
  );
}
...
function update(updateMedlem) {
  ...
  var values = sheet
    .getRange(1, 1, sheet.getLastRow(), LASTRAWDATA_COLUMNID)
    .getValues();
  var rowIndex = swcadmin_common.getIndexOf(values, function(row) {
    return row[0] == updateMedlem.medlemsnummer;
  });
  ...
  targetMemberData[BRUGERKLUBUDSTYR_COLUMN_ID - 1] =
    updateMedlem.brugerklubudstyr;
...
function getNytMedlemsnummer() {
  ...
  var result = sortedMedlemsnummer[sortedMedlemsnummer.length - 1] + 1;

  return result;
}
function getMedlemmerStamdata() {
  var sheet = SpreadsheetApp.openById(_MEDLEMMER_SHEET_ID).getSheetByName(
    _MEDLEMMER_SHEET_NAME
  );
  return sheet.getRange(2, 1, sheet.getLastRow(), 10).getValues();
}
...
function erUdmeldt(email, stamdata) {
  ...
  var result = stamdata.filter(function(row) {
...
function erMedlem(email, stamdata) {
  ...
  var result =
    stamdata.filter(function(row) {
      return (
        row[MAIL_COLUMN_ID - 1].toLowerCase() == email.toLowerCase() &&
        row[UDMELDT_COLUMN_ID - 1] == ""
      );
    }).length > 0;
  return result;
}
...
function getMedlemsNummmer(email, stamdata) {
  ...
  var memberInfo = stamdata.filter(function(row) {
    return (
      row[MAIL_COLUMN_ID - 1].toLowerCase() == email.toLowerCase() &&
      row[UDMELDT_COLUMN_ID - 1] == ""
    );
  ...
  var nummer =
    memberInfo.length > 0 ? memberInfo[0][MEDLEMSNUMMER_COLUMN_ID - 1] : -1;
  return nummer;
}
function harHylde(email) {
  var medlemsNummer = getMedlemsNummmer(email);
  var hylder = getHyldeDataValues().filter(function(row) {
...
function setUdmeldtStatus(email) {
  ...
  var emails = sheet
    .getRange(startRowOffset, MAIL_COLUMN_ID, lastRow, 1)
    .getValues();

  var rowIndex =
    swcadmin_common.getIndexOf(emails, function(row) {
      return row[0].toLowerCase() == email.toLowerCase();
    }) + startRowOffset;
...
var HYLDESHEET_ID = PropertiesService.getScriptProperties().getProperty(
  "HylderSheetId"
);
