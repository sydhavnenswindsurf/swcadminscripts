...
var HYLDE_VENTELISTE_SHEET_ID= PropertiesService.getUserProperties().getProperty("HyldeVentelisteSheetId"); 
...
function loadVenteliste(){
  ...
  .map(function(row){
    return { 
      ...
      tidsstempel:row[2] instanceof Date? Utilities.formatDate(row[2], "GMT", "dd/MM/yyyy"):row[2]
    }
  });
  return result;
...
function setStatus(email,status){
  ...
  sheet.getRange(rowIndex+1, STATUS_COL).setValue(status);
}
